---
title: "Global Life Expectancy Exploration"
...
```{r}
life = read_csv(file = "/Users/dpcle/Downloads/Life Expectancy Data.csv", show_col_types = FALSE)

```

```{r}
life_expectancy = life %>%
  drop_na(`Year`, `Life expectancy`) %>% mutate(`Life expectancy` = strtoi(`Life expectancy`)) %>% 
  select(`Year`, `Life expectancy`, `Status`) %>% 
...
```{r}
life_expectancy %>% 
  ggplot(aes(x = Year, y = Average_Life_Expectancy)) + geom_point() + geom_smooth(method = 'loess', formula = 'y ~ x', se = FALSE) + facet_wrap(vars(Status)) + ggtitle("Developed vs. Developing Countries' Life Expectancy from 2000 to 2015") + ylab("Average Life Expectancy (Years)")
```
...
$$
F is a generic distribution for the population of differences $\triangle$ is the mean of this distribution $\sigma$ is the standard deviation
...
We did not want to assume equal variance, so we performed a Welch two sample t-test. This type of t-test calculates the denominator of the t statistic differently, and also adjusts the degrees of freedom to compensate.
Below is the pvalue we calculated. 
```{r}
x = life %>% 
...
y = life %>% 
  filter(Status == "Developing") %>% drop_na() %>% 
  pull(`Life expectancy`) 
...
```{r}
infant_death = life %>% drop_na(`Year`, `infant deaths`) %>% mutate(`infant deaths` = strtoi(`infant deaths`)) %>% 
  select(`Year`, `infant deaths`, `Status`) %>% 
  group_by(Status, `Year`) %>% 
  summarise(`average infant deaths` = mean(`infant deaths`, na.rm = TRUE), .groups = 'drop')
infant_death %>% ggplot(aes(x = Year, y = `average infant deaths`)) + geom_point() + geom_smooth(method = 'loess', formula = 'y ~ x') + facet_wrap(vars(Status)) + ggtitle("Developed vs. Developing Countries' Infant Deaths from 2000 to 2015") + ylab("Average Infant Death Count per 100,000")
```
...
```{r}
adult_death = life %>% drop_na(`Year`, `Adult Mortality`) %>% mutate(`Adult Mortality` = strtoi(`Adult Mortality`)) %>% 
  select(`Year`, `Adult Mortality`, `Status`) %>% 
  group_by(Status, `Year`) %>% 
  summarise(`average Adult Mortality` = mean(`Adult Mortality`, na.rm = TRUE), .groups = 'drop')

adult_death %>% ggplot(aes(x = Year, y = `average Adult Mortality`)) + geom_point() + geom_smooth(method = 'loess', formula = 'y ~ x') + facet_wrap(vars(Status)) + ggtitle("Developed vs. Developing Countries' Adult Mortality from 2000 to 2015") + ylab("Average Adult Death Count per 100,000")
```
...
life = life %>% mutate(status_num = case_when(
  Status == "Developing" ~ 0,
  Status == "Developed" ~ 1
))
model = lm(formula = `Life expectancy` ~ `Adult Mortality` + `infant deaths` + `status_num`, data = life)
```
...
```{r}
plot(predict(model), life$`Life expectancy`, 
     xlab = "Predicted life expectancy (yrs)", 
     ylab = "Observed life expectancy (yrs)")
